...
//  sparki.println(dTheta);
  ...
  //sparki.print("Index: ");
  //sparki.print(path_index);
  /*sparki.print("Path: ");
  for(int i=0; i<NUM_X_CELLS*NUM_Y_CELLS; i++){
    sparki.print(path[i]);
    sparki.print(" ");
  }*/
  //sparki.print(path[path_index]);
  sparki.updateLCD();
}
  ...
  */
  ...
  switch(state){
    case STATE_START:
      prev = run_dijkstra(world_map, ij_coordinates_to_vertex_index(source_i, source_j)); 
      path = reconstruct_path(prev, ij_coordinates_to_vertex_index(source_i, source_j), ij_coordinates_to_vertex_index(goal_i, goal_j));
      if(path[0]==ij_coordinates_to_vertex_index(source_i, source_j)) state = STATE_HAS_PATH;
      break;
    case STATE_HAS_PATH:
      int i;
      int j;
      vertex_index_to_ij_coordinates(path[path_index+1], &i, &j);
      ij_coordinates_to_xy_coordinates(i, j, &dest_pose_x, &dest_pose_y);
      int ci, cj;
      vertex_index_to_ij_coordinates(path[path_index], &ci, &cj);
      if(ci < i) dest_pose_theta = to_radians(0);
      if(ci > i) dest_pose_theta = to_radians(180);
      if(cj < j) dest_pose_theta = to_radians(90);
      if(cj > j) dest_pose_theta = to_radians(-90);
